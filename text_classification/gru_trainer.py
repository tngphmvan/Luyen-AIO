...
from torch.nn import CrossEntropyLoss, Linear, Dropout, Sequential, GRU, Embedding
import torch
...
class GRUModel(LightningModule):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        if attention_mask is not None:
            ...
            last_output = gru_out.gather(1, idx).squeeze(
...
if __name__ == "__main__":
    logger = TensorBoardLogger("tb_logs", name="gru-text-classification")
    comet_logger = CometLogger(
        api_key="NYszMwP9QgVU7MlrN2AsTj0vA",
        project_name="gru-text-classification",
    )
    ...
    model = GRUModel(lr=1e-3, embedding_dim=128,
