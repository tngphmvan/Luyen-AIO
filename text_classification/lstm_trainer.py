...
from torch.nn import CrossEntropyLoss, Linear, Dropout, Sequential, LSTM, Embedding
import torch
...
class LSTMModel(LightningModule):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        if attention_mask is not None:
            ...
            last_output = lstm_out.gather(1, idx).squeeze(
...
if __name__ == "__main__":
    logger = TensorBoardLogger("tb_logs", name="lstm-text-classification")
    comet_logger = CometLogger(
        api_key="NYszMwP9QgVU7MlrN2AsTj0vA",
        project_name="lstm-text-classification",
    )
    ...
    model = LSTMModel(lr=1e-3, embedding_dim=128,
